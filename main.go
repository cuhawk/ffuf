...
func ParseFlags(opts *ffuf.ConfigOptions) *ffuf.ConfigOptions {
	...
	flag.IntVar(&opts.General.PauseStatus, "pause-status", 403, "HTTP status code that triggers a pause")
	flag.DurationVar(&opts.General.PauseDuration, "pause-duration", 5*time.Minute, "Duration to pause when the status code is received")
	...
	flag.IntVar(&opts.General.PauseStatus, "pause-status", opts.General.PauseStatus, "HTTP status code to trigger pause")
	flag.IntVar(&opts.General.PauseDuration, "pause-duration", opts.General.PauseDuration, "Pause duration in minutes when pause-status is encountered")
